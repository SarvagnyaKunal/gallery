...
export function rememberedAddress() {
...
async function persistFcmToken(fcmToken) {
    if (typeof fcmToken !== "string" || !fcmToken)
        return;
    if (localStorage.getItem(LS_FCM_TOKEN) === fcmToken)
        return;
...
export async function runLocalWakeSender() {
    try {
...
function normalize(ip) {
    ...
    if (!s)
        throw new Error("Enter your phone's address");
    let host = s;
...
export async function pair(ip, otp) {
    ...
    if (!res.ok)
        throw new Error(res.status === 403 ? "Wrong code" : `Server error (${res.status})`);
...
export async function waitForPhoneServer() {
    if (!isPaired())
        return;
    ...
    if (!res.ok)
        throw new Error("Phone sharing is not available");
